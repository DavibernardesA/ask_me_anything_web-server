...
const insertIntoRoom = `-- name: InsertIntoRoom :one
INSERT INTO rooms ("theme")
VALUES ($1)
RETURNING "id"
`

func (q *Queries) InsertIntoRoom(ctx context.Context, theme string) (uuid.UUID, error) {
	row := q.db.QueryRow(ctx, insertIntoRoom, theme)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
...
func (q *Queries) InsertMessage(ctx context.Context, id uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, insertMessage, id)
